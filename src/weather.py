...
def load_epw_weather(epw_path, solar_csv_path=None):
    """Load EPW weather file and optional solar position CSV."""
    ...
    weather['DateTime'] = pd.to_datetime({
        'year': weather['Year'], 'month': weather['Month'],
        'day': weather['Day'], 'hour': weather['Hour'] - 1
    })
    weather['T_ground_C'] = weather['Month'].map(lambda m: gnd_temps[m-1])

    if solar_csv_path and os.path.exists(solar_csv_path):
        solar = pd.read_csv(solar_csv_path)
        solar['datetime'] = pd.to_datetime(solar['datetime'], format='%d/%m/%Y %H:%M')
        weather['theta_s_deg'] = solar['zenith'].values
        weather['phi_s_deg'] = solar['azimuth'].values
    else:
        loc = header[0].split(',')
        lat = float(loc[6])

        def calc_solar(lat, doy, hr):
            decl = 23.45 * np.sin(np.radians(360*(284+doy)/365))
            ha = 15 * (hr - 12)
            lat_r, dec_r, ha_r = np.radians(lat), np.radians(decl), np.radians(ha)
            elev = np.degrees(np.arcsin(
                np.sin(lat_r)*np.sin(dec_r) + np.cos(lat_r)*np.cos(dec_r)*np.cos(ha_r)))
            azim = np.degrees(np.arctan2(
                np.sin(ha_r), np.cos(ha_r)*np.sin(lat_r) - np.tan(dec_r)*np.cos(lat_r)))
            return 90.0 - elev, azim

        weather['Day_of_Year'] = weather['DateTime'].dt.dayofyear
        angles = [calc_solar(lat, d, h) for d, h in zip(weather['Day_of_Year'], weather['Hour'])]
